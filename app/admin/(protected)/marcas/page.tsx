...
export default function MarcasPage() {
  const [data, setData] = useState<{ id:string;name:string;slug:string;country:string|null;type:string;status:string }[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    getSupabaseBrowser().from("brands").select("id,name,slug,country,type,status").order("name")
        ...
        <div className="border border-gray-200 rounded-xl bg-white overflow-hidden">
          <table className="w-full text-sm">
            <thead><tr className="border-b border-gray-100 bg-gray-50">
              {["Nombre","Tipo","País","Slug","Estado",""].map(h => <th key={h} className="text-left px-4 py-3 text-xs font-medium text-gray-600 uppercase tracking-wide">{h}</th>)}
            </tr></thead>
            <tbody className="divide-y divide-gray-50">
              {data.map(r => (
                <tr key={r.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3"><Link href={`/admin/marcas/${r.id}/edit`}><Button variant="ghost" size="icon"><Pencil className="w-3.5 h-3.5"/></Button></Link></td>
                </tr>
              ))}
            </tbody>
          </table>
